      ...
      (async function () {
        try {
          ...
          if (USE_SAY_AGAIN && sayAgain) {
            ...
            // DEPRECATED: call AWS Polly directly
            logger.warn(
              `DEPRECATED:`,
            ...
            that.log(
              `serviceSynthesize() ${JSON.stringify(params)} ${request.outpath}`
            );
            polly.synthesizeSpeech(params, (err, data) => {
              if (err) {
                that.warn(`serviceSynthesize()`, request, err.stack);
                reject(err);
              } else if (data == null) {
