(function (exports) {
  ...
  const RE_PARA = new RegExp(
    `^[${Words.U_RSQUOTE}${Words.U_RDQUOTE}]*\n$`,
    "u"
  );
  const RE_PARA_EOL = /^\n\n+$/u;
  ...
  class AbstractTTS {
    ...
    synthesizeBreak(index = this.SECTION_BREAK) {
      var i = index < 0 ? this.breaks.length - index : i;
    ...
    wordInfo(word = "") {
      var { customWords, words, unknownLang } = this;
    ...
    ipa_word(ipa, word) {
      var ssml =
        `<phoneme alphabet="ipa" ph="${ipa}">${word}</phoneme>` + this.break(0);
    ...
    wordIPA(word, language) {
      var { syllabifyLength: sylLen } = this;
      var sylWord =
        sylLen && word.length >= sylLen ? this.syllabify(word) : word;
    ...
    synthesizeResponse(resolve, reject, request) {
      ...
      if (fs.existsSync(outpath)) {
        ...
        if (stats && stats.size <= this.ERROR_SIZE) {
          ...
          reject(
            new Error(
              `sound file is too short (${stats.size}): ` +
    ...
    synthesizeSSML(ssmlFragment, opts = {}) {
      ...
      if (ssmlFragment.length > this.maxSSML) {
        ...
        ssmlFragment = ssmlFragment.replace(/>[^<]+<\/phoneme/giu, "/");
        this.log(
          [
        ...
        if (ssmlFragment.length > this.maxSSML) {
          ssmlFragment = ssmlFragment.replace(/<break[^>]+>/giu, "");
          this.log(
            [
              `AbstractTts.synthesizeSSML()`,
        ...
        (async function () {
          try {
            ...
            if (result && result.length) {
              ...
              that.log(
                [`synthesizeText("${text}",`, JSON.stringify(opts), `)`].join(
                  " "
                )
              );
              text = that.pauseSSML;
    ...
    synthesizeSegment(opts = {}) {
      var { segment, language, usage, volume } = opts;
      if (segment == null) {
        return Promise.reject(
          new Error(`synthesizeSegment() segment is required`)
        );
      }
      if (language == null) {
        return Promise.reject(
          new Error(`synthesizeSegment() language is required`)
        );
      }
