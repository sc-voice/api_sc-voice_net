(function (exports) {
  ...
  class Words {
    constructor(json, opts = {}) {
      ...
      if (json == null) {
        var filePath =
          opts.filePath || path.join(WORDS_PATH, `${this.language}.json`);
      ...
      var symAcc = Object.keys(this.symbols).reduce(
        (acc, sym) => {
    ...
    static get RE_ACRONYM() {
      return /\{[^}]*\}/u;
    }

    static get U_LSQUOTE() {
      return "\u2018";
    }
    static get U_RSQUOTE() {
      return "\u2019";
    }
    static get U_LDQUOTE() {
      return "\u201C";
    }
    static get U_RDQUOTE() {
      return "\u201D";
    }
    static get U_HYPHEN() {
      return "\u2010";
    }
    static get U_APOSTROPHE() {
      return "\u02BC";
    }
    static get U_ENDASH() {
      return "\u2013";
    ...
    static get U_ELLIPSIS() {
      return "\u2026";
    }
    static get U_A_MACRON() {
      return "\u0100";
    }
    static get U_a_MACRON() {
      return "\u0101";
    ...
    static levenshtein(s, t) {
      ...
      var d = new Array(s.length + 1)
        .fill(null)
        .map(() => new Array(t.length + 1).fill(null));
    ...
    static commonPhrase(a, b, minLength = 0) {
      ...
      var c = new Array(x.length + 1)
        .fill(null)
        .map(() => new Array(y.length + 1).fill(null));
      ...
      while (1 < lcs.length) {
        if (lcs[0] == null || a.indexOf(`${lcs[0]} ${lcs[1]}`) < 0) {
          ...
          a.indexOf`${lcs[lcs.length - 2]} ${lcs[lcs.length - 1]}` < 0
        ) {
          lcs.pop();
        }
        var pat = lcs.join(" ");
        ...
        Object.keys(this.words).forEach((key) => {
          ...
          if (value == null) {
            ...
            if (keyAlts) {
              ...
              this.altMap[value] = this.altMap[key] = [value, key];
            }
          } else {
            // key is canonical
    ...
    alternatesRegExp(text) {
      ...
      var pat = words.reduce((acc, word) => {
        ...
        var wordPat = alts.length === 1 ? alts[0] : `(${alts.join("|")})`;
