(function (exports) {
  ...
  const TRANSLATORS = {
    ...
    pt: ["beisert", "guimaraes", "laera"],
    pl: ["kowaleski", "jagodzinski"],
    nd: [],
    po: [],
    is: [],
    jpn: [
  ...
  class SuttaFactory {
    ...
    sectionSutta(sutta) {
      ...
      var newSections = segments.reduce((acc, seg, i) => {
        ...
        if (segStart === 0 && scid.groups[0] === group0) {
          ...
          (segStart === 0 && scid.groups[0] !== group0) ||
          /^[1-9]/.test(seg[lang])
        ) {
          acc.push(
            new Section({
    ...
    async loadSutta(opts = {}) {
      try {
        ...
        var language = opts.language || "en";
        var autoSection =
          opts.autoSection == null ? this.autoSection : opts.autoSection;
        var plainText =
          opts.plainText == null ? this.plainText : opts.plainText;
        if (SUPPORTED_LANGUAGES[language] !== true) {
          throw new Error(`Voice does not support language: ${language}`);
        }
        ...
        for (var i = 0; !sutta && i <= translators.length; i++) {
          ...
          if (translator == null) {
            throw new Error(`Sutta not found:${JSON.stringify(opts)}`);
          }
        ...
        this.warn(`SuttaFactory: loadSutta()`, JSON.stringify(opts), e.message);
        throw e;
      }
    }
