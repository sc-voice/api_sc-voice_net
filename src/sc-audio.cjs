(function (exports) {
  ...
  class SCAudio {
    constructor(opts = {}) {
      ...
      this.urlMap =
        opts.urlMap || `https://${SC_OPUS_STORE}.sgp1.digitaloceanspaces.com`;
    ...
    static get SC_OPUS_STORE() {
      return SC_OPUS_STORE;
    }
    ...
    aeneasMapUrl(
      ...
      suid = suid.toLowerCase().replace(/\.0*/gu, ".");
    ...
    convertResponse(response, resolve, reject) {
      ...
      (async function () {
        try {
          ...
          if (srcFile === dstFile) {
            // source is in destination format
            resolve(response);
          } else {
    ...
    downloadSegmentAudio(opts = {}) {
      ...
      suttaSegId = suttaSegId.replace(/\.00*/gu, ".");
    ...
    cacheSuttaAudio(opts) {
      var { suid, language, author, reader, suttaStore, soundStore } = opts;
      var that = this;
      if (suid == null) {
        return Promise.reject(
          new Error("ScAudio.cacheSuttaAudio() suid is required")
        );
      }
      ...
      return new Promise((resolve, reject) => {
        (async function () {
          try {
            ...
            var { fragments } = await that.aeneasMap(
              suid,
              language,
              author,
              reader
