...
import { ScApi, SuttaCentralId } from 'suttacentral-api';
import { BilaraData, Seeker } from 'scv-bilara';
...
function checkSuttas(data) {
  ...
  for (let i = 0; i < data.suttaRefs.length; i++) {
    const refParts = data.suttaRefs[i].split('/');
    const sutta = data.suttas[i];
...
describe('sutta-store', () => {
  it('default ctor', () => {
    const store = new SuttaStore();
  ...
  it('custom ctor', async () => {
    ...
    loggerInstance.logLevel = 'warn';
    const autoSyncSeconds = 1;
    const store = new SuttaStore({ logger: loggerInstance, autoSyncSeconds });
  ...
  it('initialize() initializes SuttaStore', async () => {
    ...
    const autoSyncSeconds = 1;
    const store = new SuttaStore({ autoSyncSeconds });
  ...
  it('search() thig1.1/pt -ra sujato -ml1', async () => {
    ...
    const lang = 'pt';
    const suid = 'thig1.1';
    const pattern = `${suid}/${lang} -ra sujato -ml1`;
    const scApi = await new ScApi().initialize();
    const suttaFactory = new SuttaFactory({ scApi, autoSection: true });
    const store = await new SuttaStore({ scApi, suttaFactory, voice })
      .initialize();

    const { method, results } = await store.search(pattern);
  ...
  it('search() sn3.3/fr -dl en -da sujato', async () => {
    ...
    const lang = 'fr';
    const suid = 'sn3.3';
    const pattern = `${suid}/${lang} -dl en -da sujato`;
    const scApi = await new ScApi().initialize();
    const suttaFactory = new SuttaFactory({
      scApi,
      autoSection: true,
    });
    const store = await new SuttaStore({
      scApi,
      suttaFactory,
      voice,
    }).initialize();
  ...
  it("search('thig16.1') returns segmented sutta", async () => {
    ...
    const { method, results } = await store.search('thig16.1');
    should(results).instanceOf(Array);
    should(method).equal('sutta_uid');
    should.deepEqual(
      results.map((r) => r.count),
  ...
  it("search('sona') finds 'Soṇa'", async () => {
    const voice = Voice.createVoice('raveena');
    const scApi = await new ScApi().initialize();
    const suttaFactory = new SuttaFactory({
      scApi,
      autoSection: true,
    ...
    const { method, results } = await store.search('sona');
    should(results).instanceOf(Array);
  ...
  it("search('thig1.1') returns segmented sutta", async () => {
    ...
    const scApi = await new ScApi(PRODUCTION).initialize();
    const suttaFactory = new SuttaFactory({
      scApi,
      autoSection: true,
    });
    ...
    const { method, results } = await store.search('thig1.1');
    should(results).instanceOf(Array);
    should(method).equal('sutta_uid');
    should.deepEqual(
      results.map((r) => r.count),
  ...
  it('search(pattern) => multiple results', async () => {
    ...
    const { method, results } = await store.search('is the root of suffering');
    should(results).instanceOf(Array);
    should.deepEqual(
    ...
    const jsonPath = path.join(__dirname, './data/search-root-of-suffering.json');
    fs.writeFileSync(
      jsonPath,
      JSON.stringify(
        {
  ...
  it('search() => thig1.1-3/en/soma maxResults', async () => {
    const scApi = await new ScApi(PRODUCTION).initialize();
    const store = await new SuttaStore({ scApi }).initialize();

    const opts = {
      pattern: 'thig1.1-3/en/soma',
      maxResults: 2,
    };
    const { method, results } = await store.search(opts);
    should(results).instanceOf(Array);
    should(method).equal('sutta_uid');
  ...
  it('search(pattern) => regular expression results', async () => {
    ...
    const { method, results } = await store.search('is.*root .*suffering');
    should(results).instanceOf(Array);
    should(method).equal('phrase');
    should.deepEqual(
      results.map((r) => r.uid),
  ...
  it('search(pattern) => no results', async () => {
    ...
    const { method, results } = await store.search('not-in-suttas');
    should(results.length).equal(0);
  });

  it('sanitizePattern(pattern) prevents code injection attacks', () => {
    const testPattern = (pattern, expected) => {
  ...
  it('search(pattern) is sanitized', async () => {
    const store = await new SuttaStore().initialize();
    let { method, results } = await store.search('"`echo doublequote`"');
    should(results.length).equal(0);
    ({ method, results } = await store.search("'`echo singlequote`'"));
    should(results.length).equal(0);
    ({ method, results } = await store.search('${PATH}'));
    should(results.length).equal(0);
    ({ method, results } = await store.search(
      'is\tthe\rroot\nof\nsuffering'
    ));
    should.deepEqual(
      results.map((r) => r.uid),
  ...
  it('search(pattern) handles long text', async () => {
    ...
    try {
      ...
      const longstring = new Array(100)
  ...
  it('search(pattern) returns voice guid', async () => {
    ...
    const { method, results } = await store.search('root of suffering');
    should(results).instanceOf(Array);
    should.deepEqual(
      results.map((r) => r.count),
  ...
  it('search(pattern) sorts by numeric count', async () => {
    const store = await new SuttaStore().initialize();
    const { method, results } = await store.search('Sāriputta');
    should.deepEqual(
      results.map((r) => r.count),
  ...
  it('paliPattern(pattern) should return the Pali pattern', () => {
  ...
  it('search(pattern) finds romanized Pali keywords ', async () => {
    const store = await new SuttaStore().initialize();
    const res = await store.search('jhana');
    const { method, results, maxDoc, resultPattern } = res;
    should(method).equal('phrase');
    should.deepEqual(
      results.map((r) => ({
        uid: r.uid,
  ...
  it('search(pattern) finds exact Pali', async () => {
    const store = await new SuttaStore().initialize();
    const res = await store.search('abhisambuddhā');
    const { method, results, maxDoc, resultPattern } = res;
    should(method).equal('phrase');
    should(resultPattern).equal('abhisambuddhā');
  });

  it('isUidPattern(pattern) is true for sutta_uid patterns', () => {
  ...
  it('search(pattern) finds mn1/en/sujato', async () => {
    const store = await new SuttaStore().initialize();

    const data = await store.search({ pattern: 'mn1/en/sujato' });
    ...
    const sutta = data.results[0].sutta;
  ...
  it('search(pattern) finds suttas in range', async () => {
    ...
    let data = await store.search({ pattern: 'sn 45.161' });
    should.deepEqual(
      data.results.map((r) => r.uid),
      ['sn45.161']
    );

    data = await store.search({ pattern: 'sn29.9-999' });
    should.deepEqual(
      data.results.map((r) => r.uid),
    ...
    data = await store.search({ pattern: 'sn29.1-5' });
    should.deepEqual(
      data.results.map((r) => r.uid),
  ...
  it('createPlaylist(opts) creates playlist', async () => {
    const store = await new SuttaStore().initialize();
    let playlist = await store.createPlaylist({
      pattern: 'an3.76-77',
    });
    should(playlist.tracks.length).equal(4);
    should.deepEqual(
  ...
  it('maxDuration limits createPlaylist()', async () => {
    const store = await new SuttaStore({
      maxDuration: 450,
    }).initialize();
    let playlist = await store.createPlaylist({
  ...
  it('an4.6 has no HTML', async () => {
    const store = await new SuttaStore({
      maxDuration: 450,
    }).initialize();
    const { method, results } = await store.search('an4.6');
    const sutta = results[0].sutta;
    should(sutta.sutta_uid).equal('an4.6');
    const en5 = sutta.segments[5].en;
  ...
  it('nikayaSuttaIds(...) returns sutta_uids', async () => {
    ...
    let ids = await store.nikayaSuttaIds('kn', 'en', 'sujato');
    ...
    ids = await store.nikayaSuttaIds('kn');
    should(ids).instanceOf(Array);
    should(ids.length).equal(672);
    should.deepEqual(ids.slice(0, 3), KNSTART);
    should.deepEqual(ids.slice(ids.length - 3, ids.length), KNEND);

    ids = await store.nikayaSuttaIds('an');
    should(ids).instanceOf(Array);
    should(ids.length).equal(1408);

    ids = await store.nikayaSuttaIds('sn');
    should(ids).instanceOf(Array);
    should(ids.length).equal(1819);

    ids = await store.nikayaSuttaIds('dn');
    should(ids).instanceOf(Array);
    should(ids.length).equal(34);

    ids = await store.nikayaSuttaIds('mn');
    should(ids).instanceOf(Array);
    should(ids.length).equal(152);

    ids = await store.nikayaSuttaIds('nonikaya', 'yiddish', 'nobody');
    should.deepEqual(ids, []);
  });

  it("search('sn12.3') returns Deutsch", async () => {
    const voice = Voice.createVoice('Amy');
    const scApi = await new ScApi().initialize();
    const suttaFactory = new SuttaFactory({
      scApi,
      autoSection: true,
  ...
  it('loadSutta(...) returns scv-bilara sutta', async () => {
    ...
    const sutta = await store.loadSutta({
