typeof describe === "function" &&
  describe("abstract-tts", function () {
    ...
    it("wordSSML(word) returns SSML text for word", function () {
      ...
      // word ending quote
      should(tts.wordSSML(`identity’`)).equal(`identity’`);

      // words without information
      should(tts.wordSSML(`ariyasaccan’ti`)).equal(
      ...
      should(tts.wordSSML("bhikkhu")).equal(
      ...
      should(tts.wordSSML("sati")).equal(
        `<phoneme alphabet="ipa" ph="s\u0250t\u026a">sati</phoneme>${BREAK}`
      );

      // english word variant
      should(tts.wordSSML("bowed")).equal(
        `<phoneme alphabet="ipa" ph="ba\u028ad">bowed</phoneme>${BREAK}`
      );

      // hyphenated word
      should(tts.wordSSML("well-to-do")).equal(`well-to-do`);

      // acronyms
      should(tts.wordSSML("{mn1.2-en-test}")).equal(
    ...
    it("tokensSSML(text) returns array of SSML tokens", function () {
      ...
      should.deepEqual(tokens, ["Life", "is", "good", "&amp;", "happy", "."]);
    });
    ...
    it("maxSegment controls segment length", function () {
      ...
      should.deepEqual(ssml, ["a b c", "d e f", "g"]);
    ...
    it("tokenize(text) returns array of tokens", function () {
      var tts = new AbstractTTS();
      should.deepEqual(tts.tokenize("281–309"), ["281–309"]);
      should.deepEqual(tts.tokenize("1,234"), ["1,234"]);
    ...
    it("concatAudio(files) returns Opus file", async () => {
      ...
      stderr = stderr.split("\n").join("\n");
      should(stderr).match(/title\s*:\s*test_title/imsu);
      should(stderr).match(/\bartist\s*:\s*test_artist/imsu);
      should(stderr).match(/album\s*:\s*test_album/imsu);
      should(stderr).match(/album_artist\s*:\s*test_album_artist/imsu);
      should(stderr).match(/comment\s*:\s*Cover/imsu);
      should(stderr).match(new RegExp(`version\\s*:\\s*${guid}`, `msiu`));
      should(stderr).match(new RegExp(`date\\s*:\\s*${date}`, `msiu`));

      // Verify defaults
      should(stderr).match(/genre\s*:\s*Dhamma/imsu);
      ...
      should(result.file)
        .match(new RegExp(`.*${result.signature.guid}.ogg`)); // output guid
    ...
    it("concatAudio(files) returns sound file", async ()=>{
      ...
      should(fs.existsSync(result.file)).equal(true); // output file guid
      should(result.file)
        .match(new RegExp(`.*${result.signature.guid}.*`)); // output guid
      await soundStore.clearEphemerals(); // cleanup
    });
    it("concatAudio(files) returns sound file", async () => {
      ...
      should(fs.existsSync(result.file)).equal(true); // output file guid
      should(result.file).match(new RegExp(`.*${result.signature.guid}.*`)); // output guid
    ...
    it("syllabify(word) spaces word by syllable", function () {
      ...
      should(tts.syllabify("tatetitotutātītū")).equal(
      ...
      should(tts.syllabify("acchariyaabbhutasutta")).equal(
